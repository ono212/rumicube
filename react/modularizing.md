...
## 2023.04.08.next 숙제02

### 잘 모르겠는 것

- 왜 `getCurrencySign`은 퍼블릭 인터페이스이고, `getRoundUpAmount`와 `getTip`은 왜 그냥 인터페이스인지?
  - 퍼블릭 인터페이스 VS 인터페이스
  - 구분기준

## 2023.04.15.next 숙제02

국가별로 화폐 단위에 따라서 기부 금액을 계산하는 로직을 변경하려고 해. 국가별로 반올림 로직을 다르게 해줘야해.

먼저 국가가 일본일 때, 100으로 기부금을 반올림하고 일본 화폐 단위를 보여주도록 구현해볼게.

1. 일단 `Payment` 컴포넌트에 국가 코드를 의미하는 `countryCode`를 prop으로 전달할거야.
2. 그리고 기부 금액과 총액을 계산하는 `useRoundUp` 훅도 변경해줘야겠지. `total`과 `tip`을 계산하는 부분에 `countryCode`가 일본인지 아닌지에 따라 다르게 계산하도록 분기처리를 추가해줬어.
3. 또 일본일 경우 화폐 기호도 일본의 화폐로 보여줘야하니까 `Payment` 컴포넌트의 총액을 보여주는 버튼 부분도 분기처리를 추가해줘야해.
4. 메시지 포매팅을 하는 `formatCheckboxLabel` 함수도 화폐기호를 보여주는 부분에 분기 처리를 추가해줘야겠지. 끝이야.

국가가 일본인 경우를 추가하니까 벌써 코드 변경이 있는 부분이 너무 많아졌다는 걸 알 수 있어. 여러 개의 모듈을 수정해야했지. 일본뿐만이 아니라 다른 국가의 화폐와 계산로직을 추가할 경우가 있을 수 있는데 그 때마다 이렇게 변경해줘야한다면 너무 힘들 것 같아..! 이렇게 변경사항이 하나 생길 때마다 여러개의 모듈을 수정해야 하는 문제를 산탄총 수술 문제라고 해. 그렇다면 이런 경우는 어떻게 해결할 수 있을까?

다형성을 사용해서 분기 처리 로직을 대체할 수 있어. 다형성이 뭔데?

몰라.

다형성으로 어떻게 이 문제를 해결할 수 있어?

총액을 계산하는 함수(`getRoundUpAmount`)와 팁을 계산하는 메서드(`getTip`)를 가지는 `PaymentStrategy` 인터페이스를 만들어. 그리고 국가별로 이 인터페이스를 구현한 클래스를 만들어. 각 국가의 화폐는 클래스에서 각자의 메서드(`currencySign`)로 반환하도록 해. 이렇게 국가별로 서브 클래스를 만들어서 해결할 수 있단다. 근데 이렇게 서브 클래스로 구현할 수도 있지만 자바스크립트의 함수가 일급함수인 특징을 사용해서 좀 더 간단하게 구현할 수 있는 방법이 있어. 이렇게 국가마다 서브 클래스를 일일이 만들지 않아도 되는 방법이거든. 알려줄게..?

여기 `CountryPayment` 클래스가 있어. `CountryPayment` 클래스의 생성자는 파라미터로 국가의 화폐 기호, 반올림하여 계산하는 로직을 받아. `Payment` 컴포넌트의 prop으로 `strategy`를 추가해줬어. `strategy`는 `CountryPayment` 클래스의 인스턴스야. `Payment` 컴포넌트를 생성할 때 `strategy` prop부분에 국가의 화폐와 반올림계산로직을 넘겨주면 되겠지? 이렇게 하면 서브 클래스를 구현하지 않아도 돼. 그리고 화폐와 계산로직이 필요한 `useRoundUp` 훅, `Payment` 컴포넌트, `formatCheckboxLabel` 함수의 코드들을 일일이 분기처리해주지 않아도 돼. `CountryPayment` 클래스를 수정해주면 되지.
